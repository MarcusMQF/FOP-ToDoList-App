...
public class TaskDAOImpl implements TaskDAO {
    ...
    public int getTotalTasks() throws SQLException {
        String query = "SELECT COUNT(*) AS total FROM tasks WHERE user_id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    public int getCompletedTasks() throws SQLException {
        String query = "SELECT COUNT(*) AS completed FROM tasks WHERE is_complete = 1 AND user_id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, currentUserId);
            ResultSet rs = stmt.executeQuery();
    ...
    public int getPendingTasks() throws SQLException {
        String query = "SELECT COUNT(*) AS pending FROM tasks WHERE is_complete = 0 AND user_id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, currentUserId);
            ResultSet rs = stmt.executeQuery();
    ...
    public Map<String, Integer> getCategorySummary() throws SQLException {
        String query = "SELECT category, COUNT(*) AS count FROM tasks WHERE user_id = ? GROUP BY category";
