...
public class TaskManager {
    ...
    public void addTask(Scanner scanner) {
        ...
        while (true) {
            ...
            taskDAO.insertTask(title, description, dueDate.toString(), category, priority, "Incomplete");
    ...
    public void displayAnalytics() {
        try (Connection conn = DatabaseUtil.connect()) {
            String totalTasksQuery = "SELECT COUNT(*) AS total FROM vector_tasks";
            String completedTasksQuery = "SELECT COUNT(*) AS completed FROM vector_tasks WHERE completion_status = 'Completed'";
            String pendingTasksQuery = "SELECT COUNT(*) AS pending FROM vector_tasks WHERE completion_status = 'Incomplete'";
            String categorySummaryQuery = "SELECT category, COUNT(*) AS count FROM vector_tasks GROUP BY category";

            // Total tasks
            PreparedStatement totalStmt = conn.prepareStatement(totalTasksQuery);
            ResultSet totalRs = totalStmt.executeQuery();
            int totalTasks = totalRs.next() ? totalRs.getInt("total") : 0;

            // Completed tasks
            PreparedStatement completedStmt = conn.prepareStatement(completedTasksQuery);
            ResultSet completedRs = completedStmt.executeQuery();
            int completedTasks = completedRs.next() ? completedRs.getInt("completed") : 0;

            // Pending tasks
            PreparedStatement pendingStmt = conn.prepareStatement(pendingTasksQuery);
            ResultSet pendingRs = pendingStmt.executeQuery();
            int pendingTasks = pendingRs.next() ? pendingRs.getInt("pending") : 0;

            // Completion rate
            double completionRate = totalTasks > 0 ? (double) completedTasks / totalTasks * 100 : 0;

            // Task categories
            PreparedStatement categoryStmt = conn.prepareStatement(categorySummaryQuery);
            ResultSet categoryRs = categoryStmt.executeQuery();
            StringBuilder categorySummary = new StringBuilder();
            while (categoryRs.next()) {
                String category = categoryRs.getString("category");
                int count = categoryRs.getInt("count");
                categorySummary.append(category).append(": ").append(count).append(", ");
            }
            if (categorySummary.length() > 0) {
                categorySummary.setLength(categorySummary.length() - 2); // Remove trailing comma and space
            }
        ...
        System.out.println("- Task Categories: " + categorySummary.toString());
